...
void getShortestDist(vector<vector<int> > &dist) {
	int n = dist.size();
	for ( int k=0 ; k<n ; k++ ) {
		for ( int i=0 ; i<n ; i++ ) {
			for ( int j=0 ; j<n ; j++ ) {
				if ( k!=i && k!=j && i!=j && dist[i][k]+dist[k][j] < dist[i][j] ) {
...
int main(int argc, char** argv) {
    ...
    vector< vector<int> > dist(nSwitches, vector<int>(nSwitches,0));
    for ( int i=0 ; i<nSwitches ; i++ ) {
    ...
    for ( int i=0 ; i<nHosts ; i++ ) {
    		for ( int j=0 ; j<nHosts ; j++ ) {
    			int ii = i % nSwitches;
    			int jj = j % nSwitches;
    			if ( dist[ii][jj]!=maxInt ) {
				lengthCount[dist[ii][jj]-(i==j)*2]++;
				timer++;
    			}
    			else {
    				cout << i << " " << j << endl;
    ...
    for ( iterMap=lengthCount.begin() ; iterMap!=lengthCount.end() ; iterMap++ ) {
    		cout << iterMap->first+2 << " " << iterMap->second << endl;
    }
